...
namespace cms_api.Controllers
{
    ...
    public class CertificateController : Controller
    {
        ...
        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] Certificate value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Create", "certificate");
            }
            catch { }
            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient( "certificate");
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Read", "certificate");
            }
            catch { }
            ...
            try
            {
                ...
                var filter = Builders<Certificate>.Filter.Ne("status", "D");

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Certificate>.
                        Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Certificate>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));

                    if (value.permission != "all")
                        filter &= (value.permission.filterPermission<Certificate>("category"));
                }
                else
                {
                    if (!string.IsNullOrEmpty(value.category))
                        filter &= Builders<Certificate>.Filter.Eq("category", value.category);
                    else
                        if (value.permission != "all")
                        filter &= (value.permission.filterPermission<Certificate>("category"));

                    if (!string.IsNullOrEmpty(value.code)) { filter &= Builders<Certificate>.Filter.Eq("code", value.code); }
                   
                    //else { filter = filter & Builders<Certificate>.Filter.Eq("center", ""); }
                    if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<Certificate>.Filter.Eq("status", value.status); }
                    if (!string.IsNullOrEmpty(value.createBy)) { filter = filter & Builders<Certificate>.Filter.Eq("createBy", value.createBy); }
                    if (!string.IsNullOrEmpty(value.title)) { filter = filter & Builders<Certificate>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    if (!string.IsNullOrEmpty(value.description)) { filter = filter & Builders<Certificate>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                    //if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Certificate>.Filter.Regex("language", value.language); }
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<Certificate>.Filter.Eq("sequence", sequence); }
                ...
                List<Certificate> docs = col.Aggregate().Match(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] Certificate value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Update", "certificate");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (value.isNotification)
                {
                    ...
                    _ = new NotificationController().PushTopics(new Notification { to = "/topics/all", title = value.title, description = value.description, data = new DataPush { page = "certificate", code = value.code } });
                }
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] Certificate value)
        {
            try
            {
                var col = new Database().MongoClient( "certificate");
                ...
                foreach (var code in codeList)
                {
                    try
                    {
                        new Criteria { code = code, title = value.title, updateBy = value.updateBy }.WriteLog("Delete", "certificate");
                    }
                    catch (Exception ex)
                    {
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "CertificateGallery");
        ...
        public ActionResult<Response> GalleryFileCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("CertificateGalleryFile");
        ...
        public ActionResult<Response> CommentRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Comment>("CertificateComment");
        ...
        public ActionResult<Response> Update([FromBody] Comment value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("CertificateComment");
        ...
        public ActionResult<Response> CategoryCreate([FromBody] Category value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("certificateCategory");
        ...
        public ActionResult<Response> CategoryUpdate([FromBody] Category value)
        {
            ...
            try
            {
                ...
                if (!value.isActive)
                {
                    var collectionContent = new Database().MongoClient("certificate");
                ...
                if (!value.isActive)
                {
                    ...
                    var updatePermission = Builders<BsonDocument>.Update.Set("CertificatePage", false).Set("isActive", false);
        ...
        public ActionResult<Response> ReOrder([FromBody] EventCalendar value)
        {
            ...
            try
            {
                var col = new Database().MongoClient<BsonDocument>("certificate");
        ...
        public ActionResult<Response> ReportRead([FromBody] Criteria value)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Certificate>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Certificate>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));
                ...
                if (docs.Count() > 0)
                {
                    #region get master
                    // get category.
                    var colCategory = new Database().MongoClient<Certificate>("certificateCategory");
                    var filterCategory = Builders<Certificate>.Filter.Eq("isActive", true);
                    var category = colCategory.Find(filterCategory).ToList();

                    // get category.
                    var colCenter = new Database().MongoClient<Certificate>("mCenter");
                    var filterCenter = Builders<Certificate>.Filter.Eq("isActive", true);
