...
namespace cms_api.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] Employee value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Create", "employee");
            }
            catch { }
            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient( "employee");
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Read", "employee");
            }
            catch { }

            try
            {
                var col = new Database().MongoClient<Employee>("employee");
                var filter = Builders<Employee>.Filter.Ne("status", "D");

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Employee>.
                        Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Employee>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));

                    if (value.permission != "all")
                        filter &= (value.permission.filterPermission<Employee>("category"));
                }
                else
                {
                    if (!string.IsNullOrEmpty(value.category))
                        filter &= Builders<Employee>.Filter.Eq("category", value.category);
                    else
                        if (value.permission != "all")
                        filter &= (value.permission.filterPermission<Employee>("category"));

                    if (!string.IsNullOrEmpty(value.code)) { filter &= Builders<Employee>.Filter.Eq("code", value.code); }
                   
                    //else { filter = filter & Builders<Employee>.Filter.Eq("center", ""); }
                    if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<Employee>.Filter.Eq("status", value.status); }
                    if (!string.IsNullOrEmpty(value.createBy)) { filter = filter & Builders<Employee>.Filter.Eq("createBy", value.createBy); }
                    if (!string.IsNullOrEmpty(value.title)) { filter = filter & Builders<Employee>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    if (!string.IsNullOrEmpty(value.description)) { filter = filter & Builders<Employee>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                    //if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Employee>.Filter.Regex("language", value.language); }
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<Employee>.Filter.Eq("sequence", sequence); }

                    var ds = value.startDate.toDateFromString().toBetweenDate();
                    var de = value.endDate.toDateFromString().toBetweenDate();
                    if (value.startDate != "Invalid date" && value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate) && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Employee>.Filter.Gt("docDate", ds.start) & Builders<Employee>.Filter.Lt("docDate", de.end); }
                    else if (value.startDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<Employee>.Filter.Gt("docDate", ds.start) & Builders<Employee>.Filter.Lt("docDate", ds.end); }
                    else if (value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Employee>.Filter.Gt("docDate", de.start) & Builders<Employee>.Filter.Lt("docDate", de.end); }
                ...
                List<Employee> docs = col.Aggregate().Match(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                                      .Lookup("employeeCategory", "category", "code", "categoryList")
                                      .As<Employee>()
                                      .ToList();
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] Employee value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Update", "employee");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (value.isNotification)
                {
                    ...
                    _ = new NotificationController().PushTopics(new Notification { to = "/topics/all", title = value.title, description = value.description, data = new DataPush { page = "employee", code = value.code } });
                }
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] Employee value)
        {
            try
            {
                var col = new Database().MongoClient( "employee");
                ...
                foreach (var code in codeList)
                {
                    try
                    {
                        new Criteria { code = code, title = value.title, updateBy = value.updateBy }.WriteLog("Delete", "employee");
                    }
                    catch (Exception ex)
                    {
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "EmployeeGallery");
        ...
        public ActionResult<Response> GalleryFileCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("EmployeeGalleryFile");
        ...
        public ActionResult<Response> CommentRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Comment>("EmployeeComment");
        ...
        public ActionResult<Response> Update([FromBody] Comment value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("EmployeeComment");
        ...
        public ActionResult<Response> CategoryCreate([FromBody] Category value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("employeeCategory");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Category>("employeeCategory");
        ...
        public ActionResult<Response> CategoryUpdate([FromBody] Category value)
        {
            ...
            try
            {
                ...
                if (!value.isActive)
                {
                    var collectionContent = new Database().MongoClient("employee");
                ...
                if (!value.isActive)
                {
                    ...
                    var updatePermission = Builders<BsonDocument>.Update.Set("EmployeePage", false).Set("isActive", false);
        ...
        public ActionResult<Response> ReOrder([FromBody] EventCalendar value)
        {
            ...
            try
            {
                var col = new Database().MongoClient<BsonDocument>("employee");
        ...
        public ActionResult<Response> ReportRead([FromBody] Criteria value)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Employee>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Employee>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));
                ...
                if (docs.Count() > 0)
                {
                    #region get master
                    // get category.
                    var colCategory = new Database().MongoClient<Employee>("employeeCategory");
                    var filterCategory = Builders<Employee>.Filter.Eq("isActive", true);
                    var category = colCategory.Find(filterCategory).ToList();

                    // get category.
                    var colCenter = new Database().MongoClient<Employee>("mCenter");
                    var filterCenter = Builders<Employee>.Filter.Eq("isActive", true);
