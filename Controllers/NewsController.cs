...
namespace cms_api.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        public ActionResult<Response> Create([FromBody] News value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Create", "News");
            }
            catch { }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<News>.
                        Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<News>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));
        ...
        public ActionResult<Response> Update([FromBody] News value)
        {
            try
            {
                new Criteria { code = value.code, title = value.title, updateBy = value.updateBy }.WriteLog("Update", "News");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                doc["productCode"] = value.productCode;
        ...
        public ActionResult<Response> GalleryFileCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("newsGalleryFile");
        ...
        public ActionResult<Response> ReportRead([FromBody] Criteria value)
        {
            try
            {
                ...
                if (docs.Count() > 0)
                {
                    #region get master
                    // get category.
                    var colCategory = new Database().MongoClient<News>("newsCategory");
                    var filterCategory = Builders<News>.Filter.Eq("isActive", true);
                    var category = colCategory.Find(filterCategory).ToList();

                    // get category.
                    var colCenter = new Database().MongoClient<News>("mCenter");
                    var filterCenter = Builders<News>.Filter.Eq("isActive", true);
