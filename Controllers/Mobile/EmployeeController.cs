    ...
    public class EmployeeController : Controller
    {
        ...
        public ActionResult<Response> read([FromBody] Criteria value)
        {
            try
            {
                ...
                //value.statisticsCreate("employee");
                var col = new Database().MongoClient<Employee>("employee");
                var filter = Builders<Employee>.Filter.Eq("status", "A");

                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Employee>.Filter.Regex("code", value.code); }
           
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = filter & Builders<Employee>.Filter.Regex("title", value.keySearch);
                    ...
                    try
                    {
                        ...
                        value1.statisticsCreateAsync("EmployeeKeySearch");
                    }
                    catch { }
                    //END : Statistic
                }
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<Employee>.Filter.Eq("category", value.category); }

                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Employee>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<Employee>.Filter.Eq("isHighlight", value.isHighlight); }
                ...
                List<Employee> docs = col.Aggregate().Match(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                ...
                try
                {
                    ...
                    valueCategory.statisticsCreateAsync("employeeCategory");
                }
                catch { }
                //END : Statistic
        ...
        public ActionResult<Response> readAll([FromBody] Criteria value)
        {
            try
            {
                ...
                List<Employee> docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).ToList();

                //Get Category
                var colCategory = new Database().MongoClient<Category>("employeeCategory");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                ...
                else
                {
                    ...
                    //if (!string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<employeeCategory>.Filter.Regex("dateStart", value.startDate); }
                    //if (!string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<employeeCategory>.Filter.Regex("dateEnd", value.endDate); }
